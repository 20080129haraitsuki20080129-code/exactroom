...
1. トップページ →「出題する」→「新しい部屋を作る」。
   ...
   **部屋コードは自分で決められます**(英数字 3 文字以上、例: `101`、`SUGAKU2`)。
   空欄にすればサーバが自動で発行します。同じコードを知っている人が同じ部屋に入り、
   別のコードの部屋は互いに干渉しません(同時にいくつでも開けます)。
...
全ページの下に問い合わせ先を出しています(`static/*.html` の `.sitefoot`)。
差し替えるときは 4 ページすべてを直してください
(`tests/test_static.py::test_every_page_shows_the_contact` が揃っているか見ています)。
...
タイムアウトした提出は `status: timeout` と `verdict: null` になり、提出回数に数えません。
...
教室で不特定多数が使うことを前提に、次を守っています。

| 観点 | 対応 |
| --- | --- |
| **色だけに頼らない** | 判定は必ず **記号 (○ ✕ △) + ことば (正解 / 不正解 / 判定できず)** で示します。グレースケールでも、どの色覚でも区別できます |
| **配色** | 「カラーユニバーサルデザイン推奨配色セット」を基にし、文字と背景のコントラストは WCAG AA (4.5:1) 以上 |
| **文字** | UD フォント **BIZ UDPGothic** を優先(読み込めなければ端末の標準フォント)。行間 1.85、字間をわずかに広げる |
| **文字サイズ** | 画面右上で **標準 / 大 / 特大**(17 / 20 / 24px)に切り替えられます。端末ごとに記憶します |
| **タップ領域** | ボタン・入力欄は 48px 以上、チェックボックスは 24px |
| **キーボード操作** | 「本文へ移動」のスキップリンク、フォーカスは 3px の枠で常に見えます |
| **ことば** | 判定の説明は専門用語を避けた平易な日本語(「あなたの答えは、模範解答とは違う内容です。」) |
| **OS の設定を尊重** | ダークモード、`prefers-reduced-motion`(動きを減らす)、`prefers-contrast`(コントラストを上げる)に対応 |
| **読み上げ** | 記号は `aria-hidden`、ことばだけが読み上げられます。役割選択は `aria-pressed` で状態を伝えます |

`tests/test_static.py` がこれらを機械的に検証しています。
---

## 管理画面(運営者用)

「部屋コードを忘れた」という問い合わせは必ず起きます。そのたびにサーバの前に
座らずに済むよう、**ブラウザから部屋の一覧を確認できる画面**を `/admin` に用意
しています。

### 既定では存在しません

`ADMIN_SECRET` を設定しないかぎり、**ページも API もすべて 404** を返します。
「ここに管理画面がある」という手がかりすら出しません。

```bash
python -c "import secrets; print(secrets.token_urlsafe(24))"
```

生成した文字列を `.env` に書いて、サーバを再起動すれば有効になります。

```
ADMIN_SECRET=<生成した 16 文字以上の文字列>
```

**16 文字未満は「未設定」として扱い、管理画面は無効のまま**にします(起動時に警告)。
全部屋のコードを守る鍵なので、中途半端な秘密で開けるより閉じているほうが安全だからです。
無効に戻したいときは `ADMIN_SECRET=` を空にして再起動してください。

### 見えるもの / 見えないもの

| 見えるもの | 見えないもの |
| --- | --- |
| 部屋コード・部屋名・開閉の状態 | **模範解答**(サーバが返しません) |
| 問題のタイトル・公開 / 非公開・作成日時 | **提出された答案の中身** |
| 参加者の名前・提出数・最終アクセス | 出題者の秘密キー(ハッシュなので復元不能) |
| 部屋ごとの問題数 / 参加者数 / 提出数 | 復帰コード・IP(いずれもハッシュ) |

運営者は部屋の「持ち主」ではないので、**答案の中身までは見えないように**してあります。

### 総当たり対策

- 失敗時は最低 0.5 秒かける + `RATE_ADMIN_LOGIN_PER_MINUTE`(既定 10 回/分)
- 比較は `hmac.compare_digest`(応答時間から秘密の中身が漏れないように)
- セッションは `ADMIN_SESSION_TTL_HOURS`(既定 6 時間)で失効

### 手元の端末で済ませたいとき

CLI でも同じことができます(サーバのあるマシンで実行)。

```bash
python3 scripts/admin.py list-rooms
```

秘密キーの再設定は CLI だけの機能です(画面からはできないようにしてあります)。

```bash
python3 scripts/admin.py reset-secret ABC123 --secret '新しい秘密キー'
```
...
**Q. 試験中に別の名前で入り直されて、提出回数の上限をリセットされない?**
A. 既定では防げません。「設定」で**新しい名前での参加を締め切る**と、
以後はすでに参加した名前でしか入れなくなります。試験開始後にこれを切るのが確実です。
